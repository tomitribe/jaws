...
**before** `@Matches` and `@Filter` in the evaluation order.

This is a shorthand for the common case of filtering by file extension.
For more complex name patterns, use `@Matches`. For arbitrary criteria,
use `@Filter`.
...
`@Suffix` can be combined with `@Prefix`, `@Matches`, and `@Filter`.
